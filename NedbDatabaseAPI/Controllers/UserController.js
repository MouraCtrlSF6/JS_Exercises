...
class UserController {
  ...
  async getAllUsers() {
    await Users.find()
      ...
      .exec((error, users) => {
        ...
        return this.response.status(200).json({ status: 200, message: users });
      })
  }

  async storeUser() {
    await Users.findOne({ email: this.request.body.email }).exec(
      (error, user) => {
        if (error)
          return this.response
            .status(400)
    ...
    for (const [key, value] of Object.entries(newUser)) {
      if (value === undefined && key !== "photo" && key !== "gender")
        return this.response
          ...
          .json({
            status: 400,
            message: `Required field '${key}' not provided`,
          });
    }
  ...
  async updateUser() {
    let newUser, oldUser;
    await Users.findOne({ _id: this.request.params.id }).exec(
      async (error, user) => {
        if (error)
          return this.response
            .status(400)
